...
const listSlice = createSlice({
	...
	reducers: {
		...
		updateMemberOfCard: (state, action) => {
			const { listId, cardId, memberId, memberAvatar, memberName, memberColor } = action.payload;
			state.allLists = state.allLists.map((list) => {
				if (list._id === listId) {
					list.cards = list.cards.map((card) => {
						if (card._id === cardId) card.members.unshift({ user: memberId, avatar: memberAvatar, name: memberName, color:memberColor });
						return card;
					});
				}
				return list;
